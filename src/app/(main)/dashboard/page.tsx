...
export default function DashboardPage() {
  ...
  const statusColors = {
    ...
    gray: { bg: 'bg-[var(--bg-tertiary)]', text: 'text-[var(--text-secondary)]', dot: 'bg-[var(--text-muted)]' },
  };
      ...
      <nav className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-8">
          ...
          <div className="hidden md:flex items-center gap-1">
            <Link
              href="/dashboard"
              className="px-3 py-2 text-sm font-medium text-[var(--text-primary)] bg-[var(--bg-tertiary)] rounded-lg"
            >
              Dashboard
            </Link>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3">
            {data.user.avatar ? (
              ...
              <div className="w-8 h-8 rounded-full bg-[var(--accent-primary)] flex items-center justify-center text-sm font-medium text-white">
                {data.user.username.charAt(0).toUpperCase()}
              </div>
            )}
            <span className="text-sm font-medium text-[var(--text-primary)] hidden sm:block">{data.user.username}</span>
          </div>
          <button
            onClick={handleLogout}
            className="px-3 py-1.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)] rounded-lg transition"
          >
            Logout
          </button>
        </div>
        ...
        <main className="max-w-4xl mx-auto px-6 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {products.map((product) => (
              <div
                key={product.slug}
                className="bg-[var(--bg-secondary)] rounded-2xl border border-[var(--border-primary)] overflow-hidden hover:border-[var(--border-hover)] transition"
              >
                <div className="p-6">
          ...
          <div className="text-center">
            <Link
              href="/pricing"
              className="text-[var(--accent-primary)] hover:text-[var(--accent-hover)] transition flex items-center justify-center gap-2"
            >
            ...
            <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-primary)] p-6 mb-8">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-[var(--text-primary)]">{data.subscription.productName} Plan</p>
                    <p className="text-sm text-[var(--text-secondary)]">${data.subscription.price}/month</p>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {isMainDomain && data.tenant && (
              <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-primary)] overflow-hidden">
                <div className="p-6 border-b border-[var(--border-primary)]">
                  <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-1">Your Portal</h2>
                  <p className="text-sm text-[var(--text-secondary)]">Access and manage your support portal</p>
                </div>

                <div className="p-6">
                  {/* Portal URL */}
                  <div className="flex items-center justify-between p-4 bg-[var(--bg-primary)] rounded-lg mb-6">
                    <div>
                      <p className="text-xs text-[var(--text-muted)] mb-1">PORTAL URL</p>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-4 py-2.5 bg-[var(--accent-primary)] hover:bg-[var(--accent-hover)] rounded-lg text-sm font-medium transition text-white flex items-center gap-2"
                    >
                      Open Portal
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      ...
                      <h3 className="font-medium text-[var(--text-primary)] mb-0.5">Integrations</h3>
                      <p className="text-xs text-[var(--text-muted)]">Jira, Hygraph & more</p>
                    </Link>
            ...
            {!isMainDomain && (
              <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-primary)] overflow-hidden">
                <div className="p-6 border-b border-[var(--border-primary)]">
                  <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-1">Quick Access</h2>
                  <p className="text-sm text-[var(--text-secondary)]">Jump to your most used features</p>
                </div>
                ...
                <div className="p-6 grid grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <h3 className="font-medium text-[var(--text-primary)] mb-0.5">Help Center</h3>
                    <p className="text-xs text-[var(--text-muted)]">Browse articles & guides</p>
                  </Link>
                  ...
                  >
                    ...
                    <h3 className="font-medium text-[var(--text-primary)] mb-0.5">Submit Ticket</h3>
                    <p className="text-xs text-[var(--text-muted)]">Get help from support</p>
                  </Link>
                  ...
                  >
                    ...
                    <h3 className="font-medium text-[var(--text-primary)] mb-0.5">Contact Us</h3>
                    <p className="text-xs text-[var(--text-muted)]">Get in touch</p>
                  </Link>
                  ...
                  >
                    ...
                    <h3 className="font-medium text-[var(--text-primary)] mb-0.5">Profile</h3>
          ...
          <div className="space-y-6">
              ...
              <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-primary)] overflow-hidden">
                ...
                <div className="p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-medium text-[var(--text-primary)]">{data.subscription.productName}</span>
                  </div>
                  <div className="flex items-center justify-between">
                  ...
                  <div className="pt-4 space-y-2">
                    <Link
                      href="/dashboard/billing"
                      className="block w-full py-2.5 text-center bg-[var(--bg-tertiary)] hover:bg-[var(--bg-elevated)] rounded-lg text-sm font-medium transition text-[var(--text-primary)]"
                    >
                      Manage Billing
                    </Link>
                    {data.subscription.cancelAtPeriodEnd ? (
            ...
            {!isMainDomain && !data.subscription && (
              <div className="bg-gradient-to-br from-[var(--accent-primary)]/20 to-[var(--accent-secondary)]/20 rounded-xl border border-[var(--accent-primary)]/20 p-5">
                ...
                <Link
                  href="/pricing"
                  className="block w-full py-2.5 text-center bg-[var(--accent-primary)] hover:bg-[var(--accent-hover)] rounded-lg text-sm font-medium transition text-white"
                >
                  View Plans
                </Link>
              </div>
            ...
            {/* Help Card */}
            <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-primary)] p-5">
              <h3 className="font-medium text-[var(--text-primary)] mb-2">Need Help?</h3>
              <p className="text-sm text-[var(--text-secondary)] mb-4">
              ...
              <div className="space-y-2">
                <Link
                  href="/support"
                  className="flex items-center gap-2 text-sm text-[var(--accent-primary)] hover:text-[var(--accent-hover)] transition"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
