...
import {
  Lightning, FileText, ShareNetwork, WifiSlash, Users, Shield, Gear, Terminal,
  Warning, Question, MagnifyingGlass, Calendar, EyeSlash, WarningCircle,
  FileDashed, Plus, Lock, ChatCircle, LinkBreak, CaretRight,
  CheckCircle, PaperPlaneTilt, BookOpenText, ArrowLeft, User, Wrench
} from '@phosphor-icons/react';

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const iconMap: Record<string, React.ComponentType<any>> = {
  Zap: Lightning, FileText, Share2: ShareNetwork, WifiOff: WifiSlash, Users, Shield, Settings: Gear, Terminal,
  AlertTriangle: Warning, HelpCircle: Question, Search: MagnifyingGlass, Calendar, EyeOff: EyeSlash, AlertCircle: WarningCircle,
  FileWarning: FileDashed, Plus, Lock, MessageSquare: ChatCircle, Unlink: LinkBreak, User, Wrench
};
      ...
      <div className="animate-slide-up">
        ...
        {node.solution?.articleSlug && (
          ...
          >
            ...
            <CaretRight size={20} weight="bold" className="text-[var(--text-muted)] group-hover:text-[var(--accent-primary)] group-hover:translate-x-1 transition-all" />
          </Link>
        )}
    ...
    <div className="animate-slide-up">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {node.options?.map((option) => {
          const Icon = option.icon ? iconMap[option.icon] : CaretRight;
          return (
            <button
              key={option.nextId}
              ...
              <div className="relative flex items-center gap-4">
                {option.icon && (
                  <div className="p-3 rounded-xl bg-[var(--bg-tertiary)] group-hover:bg-[var(--accent-primary)]/10 transition-colors">
                    <Icon size={24} weight="duotone" className="text-[var(--text-muted)] group-hover:text-[var(--accent-primary)] transition-colors" />
                  </div>
                )}
